...
void setup() {
  ...
  tr = new Terrain(40, 40, 100, 0.1, 1250, path);//int cols, int rows, float cellSize, float noiseIncreRatio, float yRange, String path
    ...
    cam = new PVector(0, -height/4, height/2);
  camT = new PVector(0, -height/4, height/2);
...
void updateAltitude() {
  ...
  if (distToTl<distToBr) {
    ...
    cam.x, cam.z);
    camT.y = (ratio[0]*tr.vertices[camPjRow+1][camPjCol].y + ratio[1]*tr.vertices[camPjRow][camPjCol].y + ratio[2]*tr.vertices[camPjRow][camPjCol+1].y)-height/4;
  }
  else {
    inTl = false;
    ...
    cam.x, cam.z);
    camT.y = (ratio[0]*tr.vertices[camPjRow][camPjCol+1].y + ratio[1]*tr.vertices[camPjRow+1][camPjCol+1].y + ratio[2]*tr.vertices[camPjRow+1][camPjCol].y)-height/4;
  }
}
